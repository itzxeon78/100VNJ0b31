...
function isValidInterfaceID(iid)  
{
	if (iid.indexOf('::')<0)
	{
	var match = iid.match(/^(?:[a-f0-9]{1,4}:){3}[a-f0-9]{1,4}$/i);
	return match != null;
	}
  	idParts = iid.split(':');
  	...
  	if (substr_count(iid, ':'))
  	{
    	var match = iid.match(/^(?::|(?:[a-f0-9]{1,4}:)+):(?:(?:[a-f0-9]{1,4}:)*[a-f0-9]{1,4})?$/i);
    	return match != null;
  	} 
	return false;
}

function isValidDNSLifetime(time)
{
   var num;
...
function encodeUrl(val)
{
   ...
   for ( i = 0; i < len; i++ ) {
      if ( val.substring(i,i+1).charCodeAt(0) < 255 ) {
         ...
         // woopsie! restore.
         //For Multiple Language, The character unicode will exceed 255.
