...
function prepareWifiJSON(){
    ...
    if($("#wifi_sec")[0].value == "WEP-128Bits")
      wps.status = 'disabled';
	else
	  wps.status = 'enabled';	
...
function wifiaccept(){
	...
	if ($("#wifi_sec")[0].value == "WEP-64Bits")
	{
		...
		{
			//alert("La clave debe tener 5 o 10 caracteres.");
	...
	else if ($("#wifi_sec")[0].value == "WEP-128Bits")
	{
		...
		else if ($("#wifiPass").attr("value").length == 26 )
		{
			...
			if(!isValidASCII($("#wifiPass").attr("value"), 0))
			{
				$('#content').text("Tu contraseña wifi contiene caracteres no válidos. Recuerda que no puedes introducir espacios inicio o final de la contraseña. Tampoco introducir varios espacios consecutivos.");
				sub.click();			
				return false;
	...
	else if (($("#wifi_sec")[0].value == "WPA2PSK") || ($("#wifi_sec")[0].value == "WPAPSKWPA2PSK") )
	{
		...
		{			
			$('#content').text("Longitud clave Wifi incorrecta. Debe contener entre 8 y 63 caracteres");
			sub.click();
			return false;
		}
		else if ($("#wifiPass").attr("value").length == 64 )
		{
			...
			if(!isValidASCII($("#wifiPass").attr("value"), 1))
			{
				return false;
			}
		}
...
function updateWPAstrength(b){
	...
	if(value.indexOf(" ") == 0)
	{
		$('#content').text("Please do not enter the space character in the start of the pre-Shared key!");
		sub.click();
		return true;			
	}
	else if(value.indexOf(" ") == value.length-1)
	{
		$('#content').text("Please do not enter the space character in the end of the pre-Shared key!");
		sub.click();
		return true;				
	}	
   	if(wpapsklen < 8) {
    	//alert("Pre-Shared longitud error clave!");
         $('#content').text("Longitud clave Wifi incorrecta. Debe contener entre 8 y 63 caracteres");
	 sub.click();
	 return true;
		...
		for(i=0;i<wpapsklen;i++){
			var c=value.charAt(i);
			if(doHexCheck(c)<0){
//				alert('Error valor Pre-Shared Key Hex !');
                                $('#content').text("Error valor Pre-Shared Key Hex !");
		                sub.click();
				return true;
			}
